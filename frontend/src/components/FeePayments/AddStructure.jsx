  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.data && !err.response.data.success) {
        setError(err.response.data.error || 'Failed to add structure');
      } else {
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-3xl">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select name="course" onChange={handleChange} className="w-full border rounded-lg p-2">
          ...
          <div>
            ...
            <select name="student" value={formData.student} onChange={handleChange} className="w-full border rounded-lg p-2">
              <option value="">Select Student</option>
              {students.map((s) => (
                <option key={s._id} value={s._id}>{s.std_name}</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input type="text" name="year" placeholder="e.g. 2026" value={formData.year} onChange={handleChange} className="w-full border rounded-lg p-2" />
          </div>
          ...
          <div>
            ...
            <input type="number" name="extra" placeholder="Extra Charges" value={formData.extra} onChange={handleChange} className="w-full border rounded-lg p-2" />
          </div>

          {/* Total */}
          <div>
            <label className="block mb-1 font-medium">Total</label>
            <input type="number" name="total" placeholder="Total Amount" value={formData.total} readOnly className="w-full border rounded-lg p-2 bg-grey-100 cursor-not-allowed" />
          </div>

          {/* Submit Button */}
          <div className="col-span-1 md:col-span-2">
            <button type="submit" className="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700">Add Structure</button>
          </div>
        </form>
      </div>
    </div>
